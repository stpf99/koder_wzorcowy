...
class ImageCoder(QMainWindow):
    ...
    def encode_board(self, board, size, strategy, params):
        ...
        if remaining:
            ...
            # Ensure all positions (0 to size-1) have a frequency entry
            freq_list = [frequencies.get(i, 0) for i in range(size)]
            _, huffman_codes = build_huffman_tree(frequencies)
            # Encode remaining positions as a bit stream
            bit_stream = ""
        ...
        output = [choice_bit, len(sequences), freq_list, huffman_bytes]
    ...
    def pack_encoded_data(self, height, width, encoded_data):
        ...
        if self.encoding_strategy == "random":
            ...
            combined = (choice_bit << 1) | 1
            binary_data.append(combined)
            binary_data.extend(bit_array)
        else:
            num_sequences = encoded_data[1]
            freq_list = encoded_data[2]
            huffman_bytes = encoded_data[3]

            # 1 bit for choice_bit, 1 bit for mode (0=sequence), 14 bits for num_sequences
            ...
            # Pack frequency list with run-length encoding for zeros
            rle_freq = []
            run = 0
            for freq in freq_list:
                if freq == 0:
                    ...
                    if run > 0:
                        rle_freq.append((0, run))
                        run = 0
                    rle_freq.append((1, freq))
            if run > 0:
                rle_freq.append((0, run))
            
            binary_data.extend(struct.pack('!H', len(rle_freq)))
            for is_freq, value in rle_freq:
                binary_data.append(is_freq)
    ...
    def unpack_encoded_data(self, binary_data):
        ...
        # Unpack frequency list
        num_rle = struct.unpack('!H', binary_data[pos:pos+2])[0]
        pos += 2
        freq_list = []
        for _ in range(num_rle):
            is_freq = binary_data[pos]
            pos += 1
            value = struct.unpack('!H', binary_data[pos:pos+2])[0]
            pos += 2
            if is_freq == 0:
                freq_list.extend([0] * value)
            else:
                freq_list.append(value)
        while len(freq_list) < size:
            freq_list.append(0)
    ...
    def decode_board(self, code, expected_size=None):
        ...
        # Decode Huffman-coded remaining positions
        frequencies = {i: freq for i, freq in enumerate(freq_list)}
        ...
        if root is None:
            ...
            for bit in bit_stream:
                ...
                if current_node.value is not None:
                    ...
                    current_node = root
                    if len(remaining) == sum(freq_list):
                        break
